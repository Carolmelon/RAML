...
class DatasetFewShot(torch.utils.data.Dataset):
    def __init__(self,
                 data,
                 n_context=None,
                 question_prefix='headline:',
                 title_prefix='title:',
                 passage_prefix='context:'):
        self.data = data
        self.n_context = n_context
    ...
    def __getitem__(self, index):
        example = self.data[index]
        question = self.question_prefix + " " + example['headline']
...
class CollatorFewshot(object):
    ...
    def __call__(self, batch):
        ...
        def append_question(example):
            if example['passages'] is None:     # 没有信息，就编码问题本身
...
class CollatorFewshotSimple(object):
    def __call__(self, batch):
        assert(batch[0]['target'] != None)
        index = torch.tensor([ex['index'] for ex in batch])
        target = [ex['target'] for ex in batch]
        texts = [example['headline'] for example in batch]
        return (index, target, texts)
...
def load_data_fewshot(data_path=None):
    data = []
...
class PureTextDataset(torch.utils.data.Dataset):
    def __init__(self,
                 data):
        self.data = data
