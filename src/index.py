...
class Indexer(object):
    ...
    def __init__(self, vector_sz, n_subquantizers=0, n_bits=8):
        ...
        param = 'HNSW64'
        self.index = faiss.index_factory(vector_sz, param, measure)
    ...
    def search_knn(self, query_vectors: np.array, top_docs: int, index_batch_size=1024) -> List[Tuple[List[object], List[float]]]:
        ...
        print('index_batch_size: {}'.format(index_batch_size))
        print('size of query_vectors: {}'.format(query_vectors.shape))
        print('self.index.ntotal: {}'.format(self.index.ntotal))
